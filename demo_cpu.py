...
def equil_to_cart(res):
    gy, gx = torch.meshgrid(torch.linspace( 0.0 + 1.0 / res[0], 1.0 - 1.0 / res[0], res[0], device='cpu'), 
                            torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device='cpu'),
                            indexing='ij')
...
def distribution(norm, wo, r, rgb):  
    ...
    weight = torch.zeros(xyz.shape[0:-1], device='cpu')[..., None]
    threshald = 0.
...
class _MLP(torch.nn.Module):
    def __init__(self, cfg, loss_scale=1.0):
        ...
        self.net = torch.nn.Sequential(*net).cpu()
...
class LightDistill(nn.Module):
    def __init__(self):
        ...
        self.B0 = torch.randn((64, 3), device = 'cpu')
        self.B1 = torch.randn((64, 3), device = 'cpu')
        self.B2 = torch.randn((64, 3), device = 'cpu')
        self.B3 = torch.randn((64, 3), device = 'cpu')
        self.B4 = torch.randn((64, 3), device = 'cpu')
        self.B5 = torch.randn((64, 1), device = 'cpu')
        mlp_cfg = {
            "n_input_dims" : 128,
            "n_output_dims" : 3,
            "n_hidden_layers" : 4,
...
def sphere_2D(res):
    gy, gx = torch.meshgrid(torch.linspace( 1.0 - 1.0 / res[0], -1.0 + 1.0 / res[0], res[0], device='cpu'), 
                            torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device='cpu'),
                            indexing='ij')
    gz = torch.ones(res[0], res[0], device='cpu')
    gz = gz - gx*gx - gy*gy
    mask = gz < 0
...
def sphere2envmap_visual(gt, img, mask, idx):
    env_map = torch.from_numpy(gt).cpu()
    res = env_map.shape
...
def relighting(env_map, img, mask, idx):
    env_map = torch.from_numpy(env_map).cpu()
    res = env_map.shape
    env_map = torch.roll(env_map, int(-res[1]/2), 1)
    env_map = torch.flip(env_map, (1,))
    shape = env_map.shape
...
def load_PE_parameters(MLP, file_name):
    parameters = torch.load(file_name, map_location=torch.device('cpu'))
...
def load_sampled_pixels(sampled_pixels, file_name):
    pixel_info = torch.load(file_name, map_location=torch.device('cpu'))
...
MLP = LightDistill()
MLP.load_state_dict(torch.load('mlp_weights.pt', map_location=torch.device('cpu')))
...
with torch.no_grad():
    ...
    for i in range(split+1):  # split the stacking process to prevent cpu out of memory
        if i == 0:
